...
class _HomePageState extends State<HomePage> {
  ...
  void _configureSelectNotificationSubject() {
    selectNotificationStream.stream.listen((String? payload) async {
      ...
      if (payload != null) {
        final list = payload.split("/").toList();
        if (list.isNotEmpty) {
          if (list.first.contains("supergryd")) {
            SuperGrydRoutes.values.forEach((e) async {
              if (e.name.contains(list.last)) {
                await Navigator.of(context).push(MaterialPageRoute<void>(
                  builder: (BuildContext context) => MyPackage(
                    route: e,
                  ),
                ));
              }
              ...
              child: Column(
                children: <Widget>[
                  ...
                  if (Platform.isAndroid) ...<Widget>[
                    ...
                    // ),
                    PaddedElevatedButton(
                      buttonText: 'Show plain notification for home '
                          'lockscreen',
                      onPressed: () async {
                        await _showHomeNotification();
                      },
                    ),
                    // PaddedElevatedButton(
                    //   buttonText: 'Show plain notification for cabBooking '
                    //       'lockscreen',
                    //   onPressed: () async {
                    //     await _showCabNotification();
                    //   },
                    // ),
                    PaddedElevatedButton(
                      buttonText: 'Show plain notification for food '
                          'lockscreen',
                      onPressed: () async {
                        await _showFoodNotification();
                      },
                    ),
                    //   PaddedElevatedButton(
                    //     buttonText:
  ...
  Future<void> _showHomeNotification() async {
    const AndroidNotificationDetails androidNotificationDetails =
        AndroidNotificationDetails('your channel id', 'your channel name',
            channelDescription: 'your channel description',
            importance: Importance.max,
    ...
    const NotificationDetails notificationDetails =
        NotificationDetails(android: androidNotificationDetails);
    await flutterLocalNotificationsPlugin.show(
        id++, 'supergryd home', 'Supergryd', notificationDetails,
        payload: 'supergryd/home');
  }

  Future<void> _showCabNotification() async {
    const AndroidNotificationDetails androidNotificationDetails =
        AndroidNotificationDetails('your channel id', 'your channel name',
            channelDescription: 'your channel description',
            importance: Importance.max,
    ...
    const NotificationDetails notificationDetails =
        NotificationDetails(android: androidNotificationDetails);
    await flutterLocalNotificationsPlugin.show(
        id++, 'supergryd cab', 'Cab', notificationDetails,
        payload: 'supergryd/cab');
  }

  Future<void> _showFoodNotification() async {
    const AndroidNotificationDetails androidNotificationDetails =
        AndroidNotificationDetails('your channel id', 'your channel name',
            channelDescription: 'your channel description',
            importance: Importance.max,
    ...
    const NotificationDetails notificationDetails =
        NotificationDetails(android: androidNotificationDetails);
    await flutterLocalNotificationsPlugin.show(
        id++, 'Supergryd Food', 'Food', notificationDetails,
        payload: 'supergryd/restaurants');
  }
